  ...
  return <section className="content-view codex-storage-view">
    ...
    <div className="codex-storage-grid">
      <article className="codex-storage-card source-card">
        ...
        {inventory?.workspaceRoots.length ? <details className="codex-workspace-roots"><summary>{inventory.workspaceRoots.length} referenced workspaces</summary>{inventory.workspaceRoots.map((root) => <code key={root}>{root}</code>)}</details> : null}
      </article>
      ...
      <article className="codex-storage-card collection-card">
        ...
        {collection ? <div className="codex-collection-summary">
          ...
          <div><span>{collection.activeSessionCount + collection.archivedSessionCount} tasks</span><span>{collection.workspaceRoots.length} workspaces</span><span>{collection.fileCount} files</span><span>{formatBytes(collection.totalBytes)}</span></div>
